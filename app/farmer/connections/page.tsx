...
import {
    ArrowLeft, Search, MessageCircle, Send, Phone, Store,
    Users, Clock, CheckCheck, Smile
} from "lucide-react"
...
export default function FarmerConnectionsPage() {
    ...
    const filteredConnections = myConnections.filter(c => {
        const name = c.dhalariName.toLowerCase()
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <Card className="p-8 text-center max-w-md">
                    ...
                    <p className="text-gray-600 mb-4">{t("chat.signInFarmer")}</p>
                    <Button onClick={() => router.push("/farmer/auth")} className="bg-emerald-600 hover:bg-emerald-700">
                        {t("dashboard.signIn")}
                    </Button>
                </Card>
            </div>
    ...
    return (
        <div className="h-screen flex flex-col bg-gray-100">
            {/* Header */}
            <header className="bg-emerald-700 text-white px-4 py-3 flex items-center gap-3 shadow-md z-10">
                <Button variant="ghost" size="icon" onClick={() => router.push("/farmer/dashboard")} className="text-white hover:bg-emerald-600">
            ...
            <div className="flex-1 flex overflow-hidden">
                        ...
                        {activeTab === "connections" ? (
                            filteredConnections.length === 0 ? (
                                <div className="text-center py-12 px-4">
                                    ...
                                    <p className="text-gray-400 text-xs mt-1">{t("chat.findTradersHint")}</p>
                                    <Button
                                        variant="link"
                                ...
                                <div className="divide-y">
                                    {filteredConnections.map(conn => {
                                        ...
                                        return (
                                            <button
                                                key={conn.id}
                                                className={`w-full text-left px-4 py-3 flex items-center gap-3 hover:bg-gray-50 transition-colors ${isActive ? "bg-emerald-50 border-l-4 border-emerald-500" : ""}`}
                                            ...
                                            >
                                                ...
                                                <div className="w-12 h-12 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center text-white font-bold text-lg shrink-0">
                                                    {conn.dhalariName.charAt(0).toUpperCase()}
                                                </div>

                                                <div className="flex-1 min-w-0">
                                                    <div className="flex justify-between items-baseline">
                                                        <span className="font-semibold text-gray-900 truncate">{conn.dhalariName}</span>
                ...
                <div className={`flex-1 flex flex-col ${!selectedConnection ? 'hidden md:flex' : 'flex'}`}>
                    {!selectedConnection ? (
                        /* No chat selected */
                        <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-emerald-50 to-blue-50">
                        ...
                        <>
                            ...
                            <div className="bg-white border-b px-4 py-3 flex items-center gap-3 shadow-sm">
                                ...
                                <div className="w-10 h-10 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center text-white font-bold">
                                    {selectedConnection.dhalariName.charAt(0).toUpperCase()}
                                </div>
                                <div className="flex-1">
                                    <h3 className="font-semibold text-gray-900">{selectedConnection.dhalariName}</h3>
                                    <p className="text-xs text-gray-500 flex items-center gap-1">
                                        <Phone className="w-3 h-3" />
                                        {selectedConnection.dhalariPhone || "N/A"}
                                    </p>
                                </div>
